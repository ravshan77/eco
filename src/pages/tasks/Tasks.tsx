...
import { ArrowRightFromLine, ChevronLeft, Settings2 } from 'lucide-react';
...
export default function Tasks() {
  ...
  const onRowDoubleClick = (e:TWorkers) => navigate(`/tasks-to-employee/${e.id}`);
            ...
            <table className="w-full min-w-[640px] border-separate border-spacing-0">
              ...
              <thead className="bg-white dark:bg-black sticky top-0 z-20 shadow-md border-b-2 border-gray-400">
                <tr className="border-b h-12 border-gray-300">
                  ...
                  <th className="p-2 text-center w-[170px] border border-gray-300">Tel</th>
                  {/* <th className="p-2 text-center w-[170px] border border-gray-300">Ish telefoni</th> */}
              ...
              <tbody className="cursor-pointer">
                {data?.map((wkr, ind) => (
                  <tr key={wkr.id} onDoubleClick={() => onRowDoubleClick(wkr)} className="border-b border-gray-300">
                    ...
                    <td className="p-2 border border-gray-300"> +998 { wkr.phone_youre ? formatPhoneNumber(wkr.phone_youre) : "" } </td>
                    {/* <td className="p-2 border border-gray-300"> +998 { wkr?.phone_work ? formatPhoneNumber(wkr.phone_work) : ""} </td> */}
                    <td className="p-2 border border-gray-300"> 
                      <div className='flex justify-center'>
                        <Button type="button" onClick={() => onRowDoubleClick(wkr)} variant={"green"} className="w-12 h-full"> <ArrowRightFromLine /> </Button>
                      </div> 
                    </td>
                  </tr>
                ))}
