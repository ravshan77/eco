...
const workerSchema = z.object({
  id: z.number().min(1, "Xodim ID topilmadi, xodim profiliga qaytdan kirib chiqing"),
  birthday: z.string(),
...
export default function EditWorkerPage() {
      ...
      try {
        ...
        if (!workerRes.status) throw new Error();
          const [statesRes, regionsRes, sectionsRes, positionsRes] = await Promise.all([
            statesAPI.getAll(), 
            regionsAPI.getRegionsByStateId(Number(workerRes.resoult.state_id)),
            sectionsAPI.getAll(),
            positionsAPI.getPositionBySectionId(Number(workerRes.resoult.section_id))
          ])
          ...
          const positions = positionsRes.resoult.map(pstn => ({ id: String(pstn.id), name: pstn?.name }))
          const regions = regionsRes.resoult
          
          setOptions({ ...options, states, sections, positions, regions });
          reset(workerRes.resoult);
  ...
  const onSubmit = async (data: TWorkers) => { 
    setLoading(true);
    try {
      const response = await workersAPI.update(data);
      if (response.status) {
  ...
  return (<>
    ...
    {isDeleteModal && (<DeleteWorkerModal open={true} onOpenChange={() => setOpenDeleteModal(null)} data={isDeleteModal} /> )}

    <div className="space-y-4 min-w-[360px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Button onClick={handleGoBack} variant={"primary"} type="button" className="w-full sm:w-auto"> <ChevronLeft className="mr-2 h-4 w-4" /> Ortga </Button>
        <h2 className="text-2xl font-bold">Xodim ma'lumotlarini tahrirlash</h2>
        <span></span>
      </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField control={control} name={"birthday"} render={({ field: formField }) => 
              <FormItem className="w-28">
                ...
                <FormControl>
                  <Input type={"date"} required min={"1930-01-01"} max={new Date().toISOString().split("T")[0]} value={formField.value} disabled={loading} onChange={(e) => setValue(formField.name, e.target.value)} />
                </FormControl>
                <FormMessage>{errors[formField.name]?.message}</FormMessage>
              </FormItem>}
            />
            ...
            <FormField control={control} name="phone_youre" render={({ field: { onChange, value } }) => (
            ...
            <Button type="button" disabled={loading} onClick={() => handleDelete(getValues())} variant={"destructive"} className="w-full"> O'chirish </Button>
          </form>
        </Form>
      </Card>
    </div>
